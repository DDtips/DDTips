  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (view === "register") {
        ...
        // --- 🔔 POŠLJI TELEGRAM OBVESTILO (ADMINU) ---
        try {
            await fetch("/api/send-telegram", {
                method: "POST",
                body: JSON.stringify({
                    message: `<b>👤 NOV UPORABNIK</b>\n\n📧 ${cleanEmail(email)}\n⏳ Status: <b>Čaka na odobritev</b>\n👇 <i>Pojdi v Supabase in ga potrdi!</i>` 
                }),
            });
        } catch (tgError) {
            console.error("Napaka pri pošiljanju Telegrama:", tgError);
        }
        ...
        if (!data.session) { 
            ...
            // Uspešna registracija, preusmerimo na /pending stran
        ...
        if (data.user) {
            // 🛑 TU JE SPREMEMBA: Preverimo, če je uporabnik potrjen (is_approved)
            ...
            if (profile && profile.is_approved === true) {
                // ✅ Uporabnik JE potrjen -> Spusti ga naprej
                setMsg("Dobrodošli!"); 
                setMsgType("success"); 
                setTimeout(() => router.replace("/"), 500); 
            } else {
                // ❌ Uporabnik NI potrjen -> Pošlji ga na pending
