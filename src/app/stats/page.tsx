...
function MultiSelectField({ label, options, selected, onChange, icon }: { label: string, options: string[], selected: string[], onChange: (val: string[]) => void, icon?: React.ReactNode }) {
  ...
  return (
    <div className="space-y-1.5 relative pointer-events-auto" ref={dropdownRef}>
      <label className="flex items-center gap-1.5 text-[10px] font-bold tracking-[0.15em] uppercase text-zinc-500">
        {icon} {label}
      </label>
      <button 
        onClick={() => setIsOpen(!isOpen)}
        className="w-full px-3 py-2.5 bg-[#13151b] border border-zinc-800 rounded-xl text-zinc-200 text-xs flex items-center justify-between hover:border-emerald-500/50 hover:bg-[#1a1d24] transition-all shadow-sm"
      >
    ...
    <div className={`relative rounded-2xl border ${borderColor} ${gradient} backdrop-blur-xl overflow-hidden shadow-2xl transition-all hover:border-opacity-50 group`}>
      ...
      <div className="p-6 relative z-10">
        ...
        <div className="text-center mb-8">
          <div className={`text-5xl md:text-6xl font-mono font-bold tracking-tight ${profitColor} drop-shadow-2xl`}>
            {eur(stats.profit)}
          </div>
        ...
        {variant !== "main" && (
           <div className="flex justify-center gap-8 mb-8 text-sm font-bold text-zinc-300 border-b border-white/5 pb-6">
              <div className="flex flex-col items-center gap-1">
                 <span className="text-[9px] font-bold text-zinc-500 uppercase tracking-wider">Prematch</span>
                 <span className={`font-mono ${stats.preProfit >=0 ? "text-emerald-400":"text-rose-400"}`}>{eur(stats.preProfit)}</span>
              </div>
              <div className="w-px h-8 bg-zinc-800"></div>
              <div className="flex flex-col items-center gap-1">
                 <span className="text-[9px] font-bold text-zinc-500 uppercase tracking-wider">Live</span>
                 <span className={`font-mono ${stats.liveProfit >=0 ? "text-emerald-400":"text-rose-400"}`}>{eur(stats.liveProfit)}</span>
              </div>
           </div>
        )}
        ...
        {variant !== "main" && (
          <div className={`grid ${variant === 'betting' ? 'grid-cols-4' : 'grid-cols-3'} gap-4 pt-4 border-t border-white/5`}>
             ...
             {variant === "betting" && (
               <>
                 <StatMetric label="Povp. Kvota" value={stats.avgOdds.toFixed(2)} color="text-sky-400" icon={<Target className="w-3.5 h-3.5" />} />
                 <StatMetric label="ROI" value={`${stats.roi.toFixed(1)}%`} color={stats.roi >= 0 ? "text-emerald-400" : "text-rose-400"} icon={<TrendingUp className="w-3.5 h-3.5" />} />
               </>
             )}
  ...
  return (
    <div className="space-y-1.5 pointer-events-auto">
      ...
      <input
        ...
        className="w-full px-3 py-2.5 bg-[#13151b] border border-zinc-800 rounded-xl text-zinc-200 text-xs focus:outline-none focus:border-emerald-500/50 transition-all placeholder:text-zinc-700 text-center font-medium shadow-sm hover:border-zinc-700"
      />
    </div>
  );
}
    ...
    <div className="space-y-1.5 pointer-events-auto">
      ...
      <div className="relative">
        <select
          value={value}
          onChange={(e) => onChange(e.target.value)}
          className="w-full px-3 py-2.5 appearance-none bg-[#13151b] border border-zinc-800 rounded-xl text-zinc-200 text-xs focus:outline-none focus:border-emerald-500/50 transition-all cursor-pointer text-center font-medium shadow-sm hover:border-zinc-700"
        >
